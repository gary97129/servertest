...
class TicketViewSet(OptimizationMixin, CreateUpdateCustomMixin, CreateModelMixin, ListModelMixin, RetrieveModelMixin, GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        vdata = serializer.validated_data #校驗之後的數據
        ...
        ticket_data=save_ticket_data) # 先創建出來
        # 更新title和sn
        ...
        sn = WfService.get_ticket_sn(ticket.workflow) # 流水號
        ticket.sn = sn
        ticket.title = title
        ticket.save()
    ...
    def accpet(self, request, pk=None):
        """
        接單,當工單當前處理人實際為多個人時(角色、部門、多人都有可能， 注意角色和部門有可能實際只有一人)
        """
        ticket = self.get_object()
        ...
        if result.get('need_accept', False):
            ...
            raise APIException('無需接單')
    
    @action(methods=['post'], detail=True, perms_map={'post':'*'})
    def retreat(self, request, pk=None):
        """
        撤回工單，允許創建人在指定狀態撤回工單至初始狀態，狀態設置中開啟允許撤回
        """
        ticket = self.get_object()
        if ticket.create_by != request.user:
            raise APIException('非創建人不可撤回')
        if not ticket.state.enable_retreat:
            raise APIException('該狀態不可撤回')
    ...
    def add_node(self, request, pk=None):
        ...
        ticket.save()
        # 更新流轉記錄
        suggestion = request.data.get('suggestion', '') # 加簽說明
        TicketFlow.objects.create(ticket=ticket, state=ticket.state, ticket_data=WfService.get_ticket_all_field_value(ticket),
    ...
    def add_node_end(self, request, pk=None):
        ...
        ticket.save()
        # 更新流轉記錄
        suggestion = request.data.get('suggestion', '') # 加簽意見
        TicketFlow.objects.create(ticket=ticket, state=ticket.state, ticket_data=WfService.get_ticket_all_field_value(ticket),
    ...
    def close(self, request, pk=None):
        ...
        if ticket.state.type == State.STATE_TYPE_START and ticket.create_by==request.user:
            ...
            ticket.save()
            # 更新流轉記錄
            suggestion = request.data.get('suggestion', '') # 關閉原因
            TicketFlow.objects.create(ticket=ticket, state=ticket.state, ticket_data=WfService.get_ticket_all_field_value(ticket),
            ...
            return Response('工單不可關閉', status=status.HTTP_400_BAD_REQUEST)
