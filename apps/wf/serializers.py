...
class FieldChoiceSerializer(serializers.Serializer):
    ...
    name = serializers.CharField(label='名稱')
...
class CustomFieldCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    field_choice = FieldChoiceSerializer(label='選項列表', many=True, required=False)
    class Meta:
        model = CustomField
...
class TicketDetailSerializer(serializers.ModelSerializer):
    ...
    def get_ticket_data_(self, obj):
        ...
        for i in all_fields_l:
            ...
            i['field_display'] = i['field_value'] # 該欄位是用於查看詳情直接展示
            if i['field_value']:
...
class TicketHandleSerializer(serializers.Serializer):
    transition = serializers.PrimaryKeyRelatedField(queryset=Transition.objects.all(), label="流轉id")
    ticket_data = serializers.JSONField(label="表單數據json")
    suggestion = serializers.CharField(label="處理意見", required = False, allow_blank=True)
...
class TicketCloseSerializer(serializers.Serializer):
    suggestion = serializers.CharField(label="關閉原因", required = False)

class TicketAddNodeSerializer(serializers.Serializer):
    suggestion = serializers.CharField(label="加簽說明", required = False)
    toadd_user = serializers.PrimaryKeyRelatedField(queryset=User.objects.all(), label='發送給誰去加簽')

class TicketAddNodeEndSerializer(serializers.Serializer):
    suggestion = serializers.CharField(label="加簽意見", required = False)

class TicketDestorySerializer(serializers.Serializer):
    ids = serializers.ListField(child=serializers.PrimaryKeyRelatedField(queryset=Ticket.objects.all()), label='工單ID列表')